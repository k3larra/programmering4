...
public class Parser {
	 ...
	 * @return list of stations that fulfils the search criteria. Always returns
	 *         a number of central stations like Malmï¿½ Copenhagen etc
	 */
	...
	/**
	 * Calls Skånetrafiken API and searches for departures from a certain
	 * station to another station Use this URL to test from a browser:
	 * http://www.labs.skanetrafiken.se/v2.2/resultspage.asp?cmdaction=next&
	 * selPointFr=malmö%20C|80000|0&selPointTo=landskrona|82000|0&LastStart=
	 * 2015-02-24 16:38 More information
	 * 
	 * @param serachURL
	 *            string to search for
	 * @return a Journeys object that contains information on journeys from a
	 *         station to another specified station.
	 */
	...
	public static Journeys getJourneys(String searchURL) {
		...
		if (xml != null) {
			...
			try {
				NodeList errorList = doc.getElementsByTagName("Message");
				error = errorList.item(0).getTextContent();
				journeys.setErrorMsg(error);
			...
			if (error == null || error == "") {
				...
				for (int i = 0; i < nl.getLength(); i++) {
					...
					for (int j = 0; j < toNode.getLength(); j++) {
						...
						if (j == (toNode.getLength() - 1)) { // get the last
																// stop
					...
					for (int j = 0; j < lineNode.getLength(); j++) {
						...
						if (j == 0) {
							...
							// Here is the place to get Line for other stations
							// along the journey between startStation and
							// endStation
						}
					}
					...
					thisJourney.setDepTimeDeviation(parser.getValue(e, "DepTimeDeviation"));
					 	...
					 	if (Integer.parseInt(thisJourney.getRealDepTime()) > 0) {
					 		if(thisJourney.getStartStation().getStationName().contentEquals("Malmö Anna Lindhs plats"))
						 {
						...
						System.out.println(thisJourney.getStartStation().getStationName() + " - "
								+ thisJourney.getEndStation().getStationName()+ thisJourney.getRealDepTime());
		...
		if (xml != null) {
			...
			for (int i = 0; i < nl.getLength(); i++) { // Iterate all Line
				...
				if (debug) {
					...
					// departure time and date as String
